...
def get_balance(acct_id=None):
	...
	if not acct_id:
		try:
			...
			acct_id = ACCOUNT_ID = accounts['data'][1]['id']
...
# 	r = get_kline('btcusdt', '5min', 20)
# 	print r
# 	t_now = r.get('ts')/1000.0
# 	t_last = r.get('data')[0].get('id')
# 	print "Time(Now):",time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(t_now)),"<-->",
# 	print "Time(LastRecord):",time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(t_last)),
# 	print "[{}]".format(t_now-t_last)
print get_accounts()
# symbol_list = get_symbols().get("data", [])
# print map(lambda x:x['base-currency']+x['quote-currency'], symbol_list)